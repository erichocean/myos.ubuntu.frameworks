    ...
    CFDictionaryRef options)
{
    ...
    DLog(@"sourceClasses: %@", sourceClasses);
    DLog(@"cnt: %d", cnt);
    NSString *possibleType = [options valueForKey:kCGImageSourceTypeIdentifierHint];
    DLog(@"possibleType: %@", possibleType);
    if (possibleType) {
        for (NSUInteger i=0; i<cnt; i++) {
            Class cls = [sourceClasses objectAtIndex:i];
            DLog(@"cls: %@", cls);
            DLog(@"[cls typeIdentifiers]: %@", [cls typeIdentifiers]);
