...
size_t OPDataProviderGetBytesAtPositionCallback(CGDataProviderRef dp, void *buffer, off_t position, size_t count)
{
...
static size_t opal_DataGetBytesAtPosition(void *info, void *buffer, off_t position, size_t count)
{
...
static void opal_DataReleaseInfo(void *info)
{
    ...
    if (dataInfo->releaseData) { 
        //DLog(@"dataInfo->info retainCount: %d", [(id)dataInfo->info retainCount]);
        dataInfo->releaseData(dataInfo->info, dataInfo->data, dataInfo->size);
        //DLog(@"dataInfo->info retainCount: %d", [(id)dataInfo->info retainCount]);
    }
    free(dataInfo);
}
...
CGDataProviderRef CGDataProviderCreateWithData(
    ...
    size_t size,
    CGDataProviderReleaseDataCallback releaseData)
    //void (*releaseData)(void *info, const void *data, size))
{
