...
- (id)initWithSurface:(cairo_surface_t *)target size:(CGSize)size
{
    ...
    //I don't know why CGAffineTransformIdentity has value of zeroes
    self->txtmatrix = CGAffineTransformMake(1,0,0,1,0,0);
    self->scale_factor = 1;
...
void CGContextSetShadow(CGContextRef ctx, CGSize offset, CGFloat radius)
{    
...
void CGContextSetShadowWithColor(CGContextRef ctx, CGSize offset, CGFloat radius, CGColorRef color)
{
...
void CGContextStrokePath(CGContextRef ctx)
{
    ...
    cairo_stroke(ctx->ct);
    if (ctx->add->shadow_cp) {
        end_shadow(ctx, CGRectMake(0,0, ctx->device_size.width, ctx->device_size.height));
        //end_shadow(ctx, CGRectMake(0,0,500,500)); //FIXME
        //end_shadow(ctx, CGRectMake(0,0,1500,1500)); //FIXME
    }
    cret = cairo_status(ctx->ct);
    if (cret) {
        DLog(@"cairo_stroke status: %s", cairo_status_to_string(cret));
    }
}
...
static void fill_path(CGContextRef ctx, int eorule, int preserve)
{
    ...
    if (ctx->add->fill_cp) {
        ...
        //cairo_set_source_rgba(ctx->ct, 0.0, 0.5, 0.0, 1.0); // FIXME hardcoded
    } else {
        cairo_set_source(ctx->ct, default_cp);
    }
    if (eorule) {
    ...
    cret = cairo_status(ctx->ct);
    if (cret) {
        DLog(@"cairo_fill status: %s", cairo_status_to_string(cret));
    }
}
...
static inline void set_color(cairo_pattern_t **cp, CGColorRef clr, double alpha)
{
    ...
    //DLog(@"Set color with %0.1f %0.1f %0.1f %0.1f", (float)cc[0], (float)cc[1], (float)cc[2], (float)cc[3]*alpha);
...
{
    opal_draw_surface_in_rect(ctx, rect, opal_CGImageGetSurfaceForImage(image, cairo_get_target(ctx->ct)), opal_CGImageGetSourceRect(image));
}
    ...
    CFDictionaryRef auxiliaryInfo)
{
    ...
    // cairo_save(ctx->ct); it gets saved inside CGContextSaveGState already!
...
void CGContextEndTransparencyLayer(CGContextRef ctx)
{
    ...
    // cairo_restore(ctx->ct); it gets restored in CGContextRestoreGState
}
...
static void end_shadow(CGContextRef ctx, CGRect bounds)
{
 ...
 // writeOut(pattern);
    ...
    //cairo_surface_t *alphaSurface = cairo_image_surface_create(CAIRO_FORMAT_A8, 500, 250);
    cairo_surface_t *alphaSurface = cairo_image_surface_create(CAIRO_FORMAT_A8, bounds.size.width, bounds.size.height);
